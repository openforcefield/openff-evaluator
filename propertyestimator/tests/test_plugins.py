"""
Units tests for the propertyestimator.plugins module.
"""
...
from propertyestimator.plugins import (
...
def test_register_external_plugins(caplog):
    ...
    valid_entry_point = pkg_resources.EntryPoint.parse(
        "dummy_1 = propertyestimator.properties", dist=distribution
    )
    bad_entry_point = pkg_resources.EntryPoint.parse(
        "dummy_2 = propertyestimator.propertis", dist=distribution
    )

    # Add the mapping to the fake EntryPoint
    distribution._ep_map = {
        "propertyestimator.plugins": {
