"""
A collection of evaluator compute backends which use dask as the distribution engine.
"""
import abc
import importlib
import logging
...
class BaseDaskJobQueueBackend(BaseDaskBackend):
    """An evaluator backend which uses a `dask_jobqueue.JobQueueCluster`
...
class DaskLocalCluster(BaseDaskBackend):
    """An evaluator backend which uses a `dask` `LocalCluster`
