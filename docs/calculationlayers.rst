.. |layer_result|        replace:: :py:class:`~propertyestimator.layers.CalculationLayerResult`
.. |layer|               replace:: :py:class:`~propertyestimator.layers.CalculationLayer`
.. |layer_schema|        replace:: :py:class:`~propertyestimator.layers.CalculationLayerSchema`
.. |layer_decorator|     replace:: :py:class:`~propertyestimator.layers.calculation_layer`

.. |exception|           replace:: :py:class:`~propertyestimator.utils.exceptions.EvaluatorException`
.. |undefined|           replace:: :py:class:`~propertyestimator.attributes.UNDEFINED`

Calculation Layers
==================

A |layer| is an implementation of one calculation approach for estimating a set of physical properties,
...
A calculation layer is defined by two objects - a |layer| object which implements the main layer
logic, and a |layer_schema| which defines those settings and options exposed required by the layer.

One |layer_schema| will be provided to the for each type of property that the layer is being asked to
estimate. The base |layer_schema| currently only exposes options for optionally defining either the
...
The structure of a |layer| is relatively simple and permissive::
...
type of |layer_schema| that is associated with this layer, and a ``_schedule_calculation``. The
...
* the ``_Batch`` of properties which this layer should attempt to estimate. This object includes the properties
  to estimate, as well as the |layer_schema| for each property type.
...
The future objects returned by ``_schedule_calculation`` must return a |layer_result| object, which
includes

* the estimated property if the calculation was successful (or |undefined| otherwise).
* a list of any exceptions (of type |exception|) which were raised during the calculation.
