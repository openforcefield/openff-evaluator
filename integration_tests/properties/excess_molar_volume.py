...
def main():
    ...
    # Load in the data set containing a single excess molar volume
    # property.
    with open('binary_data_set.json') as file:
        data_set = PhysicalPropertyDataSet.parse_json(file.read())

    data_set.filter_by_property_types('ExcessMolarVolume')

    # Set up the server object which run the calculations.
    setup_server(backend_type=BackendType.LocalGPU, max_number_of_workers=1, port=8004)

    # Request the estimates.
    property_estimator = client.PropertyEstimatorClient(client.ConnectionOptions(server_port=8004))
    ...
    with open('excess_molar_volume_simulation.json', 'wb') as file:
    ...
    # Save the result to file.
    with open('excess_molar_volume_reweight.json', 'wb') as file:
