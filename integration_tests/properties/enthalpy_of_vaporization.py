...
def main():
    ...
    # Load in the data set containing a single enthalpy of vaporization
    # property.
    with open('pure_data_set.json') as file:
        data_set = PhysicalPropertyDataSet.parse_json(file.read())

    data_set.filter_by_property_types('EnthalpyOfVaporization')
    ...
    parameter_gradient_keys = [
        ParameterGradientKey(tag='vdW', attribute='epsilon', smirks='[#1:1]-[#6X4]'),
        ParameterGradientKey(tag='vdW', attribute='rmin_half', smirks='[#1:1]-[#6X4]')
    ]
    ...
    with open('enthalpy_of_vaporization_simulation.json', 'wb') as file:
    ...
    # Save the result to file.
    with open('enthalpy_of_vaporization_reweight.json', 'wb') as file:
